...
import { useState, useEffect, useRef } from 'react';
  ...
  export default function ChangeLang({size}) {
    ...
    useEffect(() => {
      ...
      return(
          <div className={`${size == "sm" ? "px-3 py-4" : "px-7 py-10 "} `}>

            <div className='realtive'>
              <div onClick={() => setShowDropdown(b => !b)} className={` cursor-pointer ${showDropdown ? "  border border-primary-light" : "bg-gray-dark"} peer  flex justify-between  w-full rounded items-center py-4 ${size == "sm" ? "px-4" : "px-8 "}`}>
                <div className='flex'>
                  <svg className={`${size == "sm" ? "mr-0" : "mr-7"} `} width="25" height="25" viewBox="0 0 25 25" fill="none" xmlns="http://www.w3.org/2000/svg">
                  ...
                  <span className={`${size == "sm" && "hidden"} text-xl`}>{locale == "en" ? "English" : "Spanish"}</span>
                </div>

                <svg className={`${size == "sm" && "hidden"} `}  width="17" height="10" viewBox="0 0 17 10" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M1 1L8.5 8.5L16 1" stroke="#A5ACBD" idth="2"/>
                </svg>
              </div>

              <div className={`${size == "sm" ? "" : "relative"} `}>
              {showDropdown && (
                <div ref={dropdown} className={`${size == "sm" ? "left-[100%] top-0" : " block"} bg-gray-dark absolute   px-2 py-2 rounded w-full  `}>
                {locales.map((l, i) => {
                  return (
                    <span className='block w-full' key={i} >
                      <Link href={asPath} locale={l}>
                        <span className={`text-sm text-left   w-full flex flex-col px-2 py-1 cursor-pointer rounded ${l === locale ? "text-primary bg-dark" : ''}`} >
